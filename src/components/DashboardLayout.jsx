        ...
        <div className="profile-card-lg">
          ...
          <p className="subtitle">
            {profile.student_type
              ? profile.student_type.toUpperCase()
              : "STUDENT"}
          </p>
        </div>
        <div className="info-grid">
          <div className="info-item">
            ...
            {isEditing ? (
              ...
              <span>{profile.program || "Not set"}</span>
            )}
          </div>
          <div className="info-item">
            <label>Contact</label>
          ...
          <div className="info-item">
            ...
            {isEditing ? (
              ...
              <span>{profile.dob ? profile.dob.split("T")[0] : "Not set"}</span>
            )}
          </div>
              ...
              <div className="info-item">
                ...
                {isEditing ? (
                  <input
                    ...
                    onChange={(e) =>
                      setFormData({ ...formData, last_school: e.target.value })
                    }
                  />
                ) : (
                  <span>{profile.last_school || "Not set"}</span>
                )}
              </div>
              <div className="info-item">
                ...
                {isEditing ? (
                  ...
                  <span>{profile.current_level || "Not set"}</span>
                )}
              </div>
            </>
          )}
...
function EnrollmentModalContent({ closeModal, user }) {
  ...
  // Fetch enrollment data for the logged-in student
  useEffect(() => {
    fetch("/api/enrollments")
      .then((res) => res.json())
      .then((data) => {
        const myEnrolled = data.filter((e) => e.student_email === user.email);
      ...
      <div className="modal-body">
        <div className="enroll-status" style={{ marginBottom: "15px" }}>
          {enrollments.length > 0 ? (
            ...
            >
              ⚠️ Not Officially Enrolled
            </span>
          )}
        </div>
        {enrollments.length === 0 ? (
          <div style={{ textAlign: "center", padding: "20px" }}>
            You have no enrolled subjects or pending requests yet.
          </div>
        ) : (
          <table className="grades-table">
            <thead>
            ...
            <tbody>
              {enrollments.map((s, i) => (
                <tr key={i}>
                  ...
                  <td>
                    <span
                      className={`status-badge ${s.status === "enrolled" ? "passed" : "progress"}`}
