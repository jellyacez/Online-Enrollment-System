...
const ENROLLED_SUBJECTS = [
  { code: "IT103", desc: "Data Structures", units: 3, schedule: "Mon/Wed 8:00-9:30 AM", instructor: "Prof. Santos" },
  { code: "IT104", desc: "Web Development", units: 3, schedule: "Mon/Wed 10:00-11:30 AM", instructor: "Prof. Reyes" },
  { code: "MATH02", desc: "Discrete Math", units: 3, schedule: "Tue/Thu 8:00-9:30 AM", instructor: "Prof. Garcia" },
  { code: "ENG102", desc: "Technical Writing", units: 3, schedule: "Tue/Thu 1:00-2:30 PM", instructor: "Prof. Lim" },
  { code: "PE002", desc: "Team Sports", units: 2, schedule: "Fri 7:00-9:00 AM", instructor: "Coach Mendoza" },
];
...
function CalendarWidget() {
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    cells.push(
      <span key={d} className={`cal-day${d === dayNum ? " cal-today" : ""}`}>{d}</span>
    );
  }

  return (
    <div className="calendar-widget">
      <div className="cal-header">{monthName} {year}</div>
...
export default function Homepage() {
  ...
  const openModal = (name) => { setActiveModal(name); setSidebarOpen(false); };
  ...
  return (
    <div className="app-container">
      ...
      <div className={`sidebar ${sidebarOpen ? "open" : ""}`}>
        ...
        <nav className="sidebar-nav">
          {dashboardCards.map((c) => (
            <button key={c.key} className="sidebar-link" onClick={() => openModal(c.key)}>
      ...
      {activeModal === "profile" && (
        <div className="modal">
          <div className="modal-header"><h2>👤 My Profile</h2><button onClick={closeModal}>✕</button></div>
        ...
        <div className="modal modal-wide">
          ...
          <div className="modal-body">
            <table className="grades-table">
              <thead>
                <tr><th>Code</th><th>Description</th><th>Units</th><th>Grade</th><th>Status</th></tr>
              </thead>
              <tbody>
                {SAMPLE_GRADES.map((g) => (
                  <tr key={g.code}>
        ...
        <div className="modal">
          ...
          <div className="modal-body">
            ...
            {SAMPLE_MEDICAL.checkups.map((c, i) => (
              <div key={i} className="announce-item"><small>{c.date}</small><p>{c.findings}</p></div>
            ))}
          </div>
        </div>
      )}
      ...
      {activeModal === "calendar" && (
        <div className="modal">
          ...
          <div className="modal-body">
            ...
            <table className="grades-table">
              <thead>
                <tr><th>Code</th><th>Subject</th><th>Day</th><th>Time</th><th>Room</th></tr>
              </thead>
              <tbody>
        ...
        <div className="modal">
          ...
          <div className="modal-body">
            <div className="balance-summary">
              <div className="bal-item"><label>Tuition</label><span>₱{SAMPLE_BALANCE.tuition.toLocaleString()}</span></div>
              <div className="bal-item"><label>Misc Fees</label><span>₱{SAMPLE_BALANCE.misc.toLocaleString()}</span></div>
              <div className="bal-item"><label>Lab Fees</label><span>₱{SAMPLE_BALANCE.lab.toLocaleString()}</span></div>
              <div className="bal-item total"><label>Total</label><span>₱{SAMPLE_BALANCE.total.toLocaleString()}</span></div>
            ...
            <small className="due-label">Next Payment Due: {SAMPLE_BALANCE.due}</small>
            <h4 className="section-title">Payment History</h4>
            {SAMPLE_BALANCE.payments.map((p, i) => (
              <div key={i} className="med-item"><strong>₱{p.amount.toLocaleString()}</strong><span>{p.date}</span><span>{p.method}</span><small>{p.ref}</small></div>
            ))}
          </div>
        </div>
      )}
      ...
      {activeModal === "enrollment" && (
        <div className="modal modal-wide">
          <div className="modal-header"><h2>📝 Enrollment</h2><button onClick={closeModal}>✕</button></div>
          <div className="modal-body">
            ...
            <table className="grades-table">
              <thead>
                <tr><th>Code</th><th>Description</th><th>Units</th><th>Schedule</th><th>Instructor</th></tr>
              </thead>
              <tbody>
                {ENROLLED_SUBJECTS.map((s) => (
                  <tr key={s.code}><td><strong>{s.code}</strong></td><td>{s.desc}</td><td>{s.units}</td><td>{s.schedule}</td><td>{s.instructor}</td></tr>
                ))}
              </tbody>
              <tfoot>
                <tr><td colSpan="2"><strong>Total Units</strong></td><td><strong>{ENROLLED_SUBJECTS.reduce((a, b) => a + b.units, 0)}</strong></td><td colSpan="2"></td></tr>
              </tfoot>
            </table>
          </div>
        </div>
